...
$totalNoLeidos = ControladorMensajes::crtContarMensajesNoLeidos($idUsuarioActual);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $accion !== null) {
  echo "<script>setTimeout(function(){ window.location.href = 'index.php?r=bandeja-entrada&c=mensajes'; }, 650);</script>";
}
?>
              ...
              <ul class="nav nav-pills flex-column">
                <li class="nav-item active">
                  <a href="index.php?r=bandeja-entrada&c=mensajes" class="nav-link">
                    ...
                    <span class="badge bg-primary float-right"><?php echo (int) $totalNoLeidos; ?></span>
                  </a>
                </li>
                <li class="nav-item">
        ...
        <div class="col-md-9">
          <div class="card card-primary card-outline">
            <div class="card-header d-flex justify-content-between align-items-center">
              <h3 class="card-title mb-0">Bandeja de entrada</h3>
              <div class="text-muted small"><?php echo (int) $totalNoLeidos; ?> sin leer</div>
            </div>
            <div class="card-body p-0">
                  ...
                  <tbody>
                    ...
                    <?php foreach ($mensajes as $mensaje): ?>
                      <?php
                        ...
                        $esLeido = (int) ($mensaje['leido'] ?? 0) === 1;
                      ?>
                      <tr class="<?php echo $esLeido ? '' : 'font-weight-bold'; ?>">
                        <td style="width: 34px;">
                          <div class="icheck-primary">
                            <input type="checkbox" value="" id="check-in-<?php echo (int) $mensaje['idMensajeParticipante']; ?>">
                            <label for="check-in-<?php echo (int) $mensaje['idMensajeParticipante']; ?>"></label>
                          </div>
                        </td>
                        <td class="mailbox-name">
                          <a href="index.php?r=detalle-mensaje&idMensaje=<?php echo (int) $mensaje['id_mensaje']; ?>" class="<?php echo $esLeido ? 'text-dark' : 'text-dark'; ?>">
                            De: <?php echo htmlspecialchars(trim(($mensaje['nombreUsuario'] ?? '') . ' ' . ($mensaje['apellidoUsuario'] ?? '')), ENT_QUOTES, 'UTF-8'); ?>
                          </a>
                        </td>
                          ...
                          <div class="btn-group btn-group-sm">
                            ...
                            <form method="post" class="d-inline">
                              ...
                              <button type="submit" name="accion" value="<?php echo $esLeido ? 'marcar_no_leido' : 'marcar_leido'; ?>" class="btn btn-default" title="<?php echo $esLeido ? 'Marcar no leido' : 'Marcar leido'; ?>">
                                <i class="fas fa-<?php echo $esLeido ? 'envelope-open' : 'envelope'; ?>"></i>
                              </button>
                            </form>
