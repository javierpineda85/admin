# Modulos

## Usuarios

- Alta, edicion, baja logica y reactivacion.
- Perfil propio y perfil de terceros.
- Foto de perfil, fecha de alta, fecha de baja y ultima conexion.
- Listados de activos, inactivos y no conectados.

## Cursos

- CRUD administrativo.
- Vista de listado con tarjetas para estudiantes y docentes.
- Detalle del curso con sus materias.
- Inscripcion de estudiantes solo desde el administrador.

## Materias

- CRUD de secciones / materias.
- Asignacion de curso, docente y tutor.
- Banner opcional o degradado por defecto.
- Vista detalle con contenido, recursos, tareas y foro.
- Vista separada de calificaciones.

## Lecciones

- Tipos soportados:
  - `MATERIAL`: lectura.
  - `TAREA`: entrega de archivo.
  - `PREGUNTA`: foro o discusion.
- Adjuntos y enlaces asociados.
- Edicion y eliminacion.
- Resumen de actividad por seccion.

## Calificaciones

- Nota numerica.
- Devolucion textual.
- Consulta por seccion y por estudiante.
- Vista separada para seguimiento academico.

## Mensajes

- Bandeja de entrada, enviados y papelera.
- Modo multiple destinatario.
- Adjuntos en varios archivos.
- Mensajes por seccion para docentes y administradores.
- Marcado como leido / no leido.

## Panel

- Indicadores por rol.
- Mensajes sin leer.
- Actividad reciente.
- Seguimiento academico y docente.
- Usuarios conectados recientemente.

## Autenticacion

- Login.
- Recuperacion de contraseña.
- Logout.
- Redireccion por rol.
